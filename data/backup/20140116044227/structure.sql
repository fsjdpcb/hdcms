...
$db->exe("DROP TABLE IF EXISTS `".$db_prefix."tag_copy`");
$db->exe("CREATE TABLE `".$db_prefix."tag_copy` (
  `tid` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `tag_name` varchar(30) DEFAULT '' COMMENT 'tag字符',
  `total` mediumint(9) DEFAULT '1' COMMENT '次数',
  PRIMARY KEY (`tid`),
...
$db->exe("DROP TABLE IF EXISTS `".$db_prefix."test`");
$db->exe("CREATE TABLE `".$db_prefix."test` (
  `aid` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '主键',
  `cid` smallint(5) unsigned NOT NULL DEFAULT '0' COMMENT '栏目cid',
  `title` char(100) NOT NULL DEFAULT '' COMMENT '标题',
  `new_window` tinyint(1) NOT NULL DEFAULT '0' COMMENT '新窗口打开',
...
$db->exe("DROP TABLE IF EXISTS `".$db_prefix."test_data`");
$db->exe("CREATE TABLE `".$db_prefix."test_data` (
  `aid` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '文章主表ID',
  `content` text COMMENT '正文',
  `images` mediumtext,
  `oneimg` varchar(100) NOT NULL DEFAULT '',
  `lianix` varchar(255) NOT NULL DEFAULT '',
  `cc` text,
  KEY `aid` (`aid`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8");
...
$db->exe("CREATE TABLE `".$db_prefix."upload` (
  ...
  KEY `uid` (`uid`),
  KEY `aid` (`aid`,`cid`,`mid`)
) ENGINE=MyISAM AUTO_INCREMENT=99 DEFAULT CHARSET=utf8 COMMENT='上传文件'");
