...
class ArticleControl extends PublicControl {
	...
	public function get_comment_num() {
		...
		if ($aid) {
			$db = K("Article");
			$comment_num =$db -> join(NULL)->where("aid={$aid}") -> getField('comment_num');
			echo "document.write($comment_num);";
			exit ;
		}
	}
	...
	public function add_favorite() {
		$data = array('mid' => $this -> _mid, 'cid' => $this -> _cid, 'aid' => $this -> _aid, 'uid' => $_SESSION['uid']);
