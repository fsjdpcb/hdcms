	...
	public function make_category() {
		...
		if (is_null($config)) {
			...
			if (is_null($category)) {
				...
				$this -> success("没有任何栏目需要生成!", $url,0.2);
			} else {
				$config = array();
				//生成所有栏目首页
				foreach ($category as $cat) {
					...
					//html文件存放根目录
					Page::$staticUrl = __ROOT__ . '/' . $this -> _html_path . str_replace(array('{catdir}', '{cid}'), array($cat['catdir'], $cat['cid']), $cat['cat_html_url']);
					Html::make("IndexControl", "category", $cat);
					//去掉页数为0时栏目
					if (!Page::$staticTotalPage)
						continue;
				...
				if (empty($config)) {
					...
					unset($_SESSION['make_all']['category']);
					unset($_SESSION['category_html_config']);
					$this -> success("所有栏目生成完毕", $url,0.2);
				}
				//储存配置到session
				session("category_html_config", $config);
				$this -> success("栏目静态初始化完成...", __METH__,0.2);
			}
		}
		//初始化完成后，生成静态
		...
		if (empty($config)) {
			...
			foreach ($config as $n => $cat) {
				for ($i = 0; $i < $cat['row']; $i++) {
					...
					$_REQUEST['mid'] = $cat['mid'];
					$_REQUEST['cid'] = $cat['cid'];
					$cat['_html'] = $this -> _html_path . str_replace(array('{catdir}', '{cid}', '{page}'), array($cat['catdir'], $cat['cid'], $_GET['page']), $cat['cat_html_url']);
					//设置分页静态变量
					Page::$staticUrl = __ROOT__ . '/' . $this -> _html_path . str_replace(array('{catdir}', '{cid}'), array($cat['catdir'], $cat['cid']), $cat['cat_html_url']);
					Html::make("IndexControl", "category", $cat);
					...
					if ($config[$n]['total_page'] < $config[$n]['self_page']) {
						unset($config[$n]);
						$this -> success("栏目{$cat['catname']}生成完毕", __METH__,0.2);
					}
				}
				//本次$cat['row']页生成完毕，执行下一轮静态生成
				$this -> success("生成栏目{$cat['catname']}的下" . ($cat['total_page'] - $cat['self_page']) . "页,
                            共有{$config[$n]['total_page']}页
                            (<font color='red'>" . floor($config[$n]['self_page'] / $config[$n]['total_page'] * 100) . "%</font>)", __METH__,0.2);
			}
		}
	}
	...
	public function make_content() {
		...
		if (is_null($config)) {
			...
			if (is_null($category)) {
				...
				$this -> success("没有任何内容需要生成!", $url,0.2);
			} else {
				$config = array();
				foreach ($category as $cat) {
					//当前栏目表
				...
				$this -> success("内容静态初始化完成...", __METH__,0.2);
			}
		}
		...
		if (empty($config)) {
			...
			foreach ($config as $n => $cat) {
				...
				foreach ($content as $con) {
					...
					$field['_html'] = Url::getContentHtml($field);
				...
				//本次$cat['row']页生成完毕，执行下一轮静态生成
				$this -> success("{$cat['catname']}共有{$cat['total_row']}条记录-
                                已经更新{$config[$n]['old_total']}条
                                (<font color='red'>" . floor($config[$n]['old_total'] / $cat['total_row'] * 100) . "%</font>)", __METH__,0.2);
			}
		}
	}
