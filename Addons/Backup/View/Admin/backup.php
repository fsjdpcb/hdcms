...
<div class="wrap">
    <div class="menu_list">
        <ul>
            <li><a href="{|U:'backup',array('app'=>'Addon')}" class="action">备份数据</a></li>
            <li><a href="{|U:'index',array('app'=>'Addon')}">备份列表</a></li>
        </ul>
    </div>
    <form action="{|U:'backup_db',array('app'=>'Addon')}" method="post"  class="hd-form" onsubmit="return backup();">
        <table class="table2">
            <thead>
            <tr>
                <td width="50">数据备份</td>
...
<script>
    ...
    function optimize() {
        if (check_select_table()) {
            hd_ajax('{|U:"optimize"}', $("[name*='table']:checked").serialize());
        }
    }
    //修复表
    function repair() {
        if (check_select_table()) {
            hd_ajax('{|U:"repair"}', $("[name*='table']:checked").serialize());
        }
    }
</script>
</body>
