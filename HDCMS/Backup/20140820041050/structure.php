...
$db->exe("CREATE TABLE `".$db_prefix."a` (
  ...
  `flag` set('热门','置顶','推荐','图片','精华','幻灯片','站长推荐','a','ff') DEFAULT NULL,
...
$db->exe("CREATE TABLE `".$db_prefix."navigation` (
  ...
  PRIMARY KEY (`nid`)
) ENGINE=MyISAM AUTO_INCREMENT=10 DEFAULT CHARSET=utf8 COMMENT='网站前台导航'");
...
$db->exe("CREATE TABLE `".$db_prefix."node` (
  ...
  PRIMARY KEY (`nid`)
) ENGINE=MyISAM AUTO_INCREMENT=52 DEFAULT CHARSET=utf8 COMMENT='节点表（后台菜单也使用）'");
...
) ENGINE=MyISAM AUTO_INCREMENT=49 DEFAULT CHARSET=utf8 COMMENT='上传文件'");
